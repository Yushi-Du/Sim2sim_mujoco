...
class DeployNode():
    ...
    def lowlevel_state_mujoco(self):
        if DEBUG and self.start_policy and SIM:
            ...
            # motor data
            self.joint_pos = np.concatenate([self.env.mj_data.qpos[7:29+7][:12], np.zeros(3), self.env.mj_data.qpos[7:29+7][15:]])
            self.obs_joint_pos = (np.array(self.joint_pos) - self.env.default_dof_pos_np) * self.env.scale_dof_pos
            self.joint_vel = np.concatenate([self.env.mj_data.qvel[6:29+6][:12], np.zeros(3), self.env.mj_data.qvel[6:29+6][15:]])
